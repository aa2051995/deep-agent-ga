...
from .assistants import PERMISSIONS, bedrock_region_prefix
...
def _bedrock_models() -> list[dict[str, Any]]:
    """Bedrock model ids, region-aware.

    Anthropic (and Meta/Mistral) models on Bedrock are invoked through a
    cross-region inference profile — the bare ``anthropic.*`` id returns
    "The provided model identifier is invalid" in regions that require a
...
def _model_providers() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "models": [
                ...
                {"name": "claude-3-5-sonnet-20241022", "label": "Claude 3.5 Sonnet v2"},
                {"name": "claude-3-5-sonnet-20240620", "label": "Claude 3.5 Sonnet v1"},
