...
**Rejoin content, not just fresh-submit content, must reach the live tail.** Switching to a thread runs the SDK's initial state fetch (`fetchStateHistory`), which for a still-executing run's checkpoint already includes everything that run produced *before* the reconnect — `stream.messages` can therefore already contain this run's own prior content by the time `joinRunStreamRef.current` fires. E2 routes `stream.messages` into `runLiveMessages[currentRunId]` filtered by `otherRunsMessageIds` (`messageMerge.ts`'s `collectOtherRunMessageIds`, recomputed fresh from every *other* run's known content — never from the current run itself), so this run's own pre-reconnect content is never excluded. An earlier design captured a one-time `liveBaselineIdsRef` snapshot at join time instead ("everything in `stream.messages` right now belongs to an earlier run") — correct for a fresh submit, but wrong here: it wrongly attributed the rejoined run's own prior content (including its own human message) to "an earlier run," leaving only tokens streamed *after* the reconnect (often none) — the run appeared to stream (`currentRunId` set, Stop button showing) with an empty transcript. See [UI Flow 6](06-browse-history.md) and `ui-state-graph.md` §10 (design failure 5) for the full mechanism.
