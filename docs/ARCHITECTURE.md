...
| `assistant_builder.py` | Maps an `AssistantConfig` to `deepagents.create_deep_agent` (model, tools, MCP, skills, memory, subagents, middleware, and tool permissions via `interrupt_on`). Uses a disk-rooted `FilesystemBackend` **only** when the assistant has skills/memory to load; otherwise the default in-memory `StateBackend` (so file tools never touch the host filesystem). |
...
- **Celery tasks** (`tasks.py`):
  ...
  - **Recursion limit** — a deep run fans out into subagents and can exceed LangGraph's default of 25 super-steps (surfacing as `GraphRecursionError`). The runtime defaults the ceiling to **50** (`DEFAULT_RECURSION_LIMIT`); set `LANGGRAPH_RECURSION_LIMIT` to tune it, and an explicit `recursion_limit` in the caller's run `config` takes precedence.
  - **Cooperative cancellation** — the only mechanism that stops an in-progress Celery run. `Service.cancel_run`'s `Scheduler.revoke(task_id, terminate=True)` cannot kill an already-running task under the `threads`/`solo` pools this project uses on Windows (Python threads can't be killed from outside; `celery.concurrency.thread.TaskPool` has no `kill_job`), so `STREAM_BACKEND_CELERY_TERMINATE_ON_CANCEL` defaults `false`. Instead, `run()`/`resume()`'s streaming loop re-fetches the run and checks `cancel_requested` every `RESEARCH_AGENT_CANCEL_POLL_INTERVAL` seconds (default 1.0, throttled — not on every streamed event); on a cancel it closes the event generator, persists a snapshot, marks the run `interrupted`, emits `lifecycle: interrupted`, and raises `RunCancelled` — a dedicated exception `worker/tasks.py` catches separately from real failures (no `error` overwrite, no autoretry). Without this, a "cancelled" Celery run kept executing to completion and silently overwrote `status=interrupted` back to `success`. See [Use Case 4](use-cases/04-cancel-run.md).
...
1. **Event bus (streaming fan-out)** — `STREAM_BACKEND_EVENT_BROKER`:
   ...
   - `rabbitmq` — `RabbitMQStreamBroker` using the **RabbitMQ Streams protocol** (`rstream`, port 5552). One durable stream per thread (`langgraphjs.stream.thread.<id>.events`), with `max-age` retention (default 12h) and offset-based replay/resume. This is what lets a Celery worker's events reach an SSE client connected to the API process.
     ...
     - **Subscribe-side failure**: unlike publishing, a fresh SSE/WebSocket subscription (`RabbitMQStreamBroker.subscribe`, called on every thread switch/reconnect) has no fallback — the client needs those events. If RabbitMQ Streams (port 5552) is unreachable, `consumer.start()` used to raise a raw `ConnectionError` straight out of `main.py`'s async generator; since that's inside an already-started `StreamingResponse`, the exception can't become a normal HTTP error — it crashes the ASGI response with an unhandled `ExceptionGroup` traceback (visible in uvicorn's log). `subscribe` now catches `(OSError, asyncio.TimeoutError)` and raises `EventBrokerUnavailable`, which every SSE/WebSocket subscribe call site in `main.py` catches and turns into a clean stream close (`retry: 5000` + a comment frame for SSE, close code 1013 for the WebSocket) — native `EventSource` clients (the UI's lifecycle monitor) treat that as a normal disconnect and auto-reconnect on their own. `GET .../join` (`join_run`, what `stream.joinStream()` actually calls to reattach to an active run — see [Use Case 3](use-cases/03-resume-run.md)) goes through the same `subscribe` via `wait_for_next_event`, but it's a long-held plain response with no client auto-reconnect to lean on, so it catches `EventBrokerUnavailable` itself and retries server-side (same as its existing `asyncio.TimeoutError` retry) instead of failing the request.
     - **Consumer-cleanup leak (fixed)**: `RabbitMQStreamSubscription.close()`'s `finally` called `discard_subscription_consumer(...)` as a plain next statement after `await self._consumer.close()`, not inside its own `finally` — if that `await` raised a `CancelledError` (e.g. the client switched threads while the SSE connection's cleanup was running under an already-cancelling anyio scope; visible in logs as rstream's own `[unsubscribe] exception in unsubscribe of Consumer: Cancelled by cancel scope ...`, which rstream itself catches via `except BaseException` and logs as a warning — expected noise, not itself a crash), `except Exception` doesn't catch it and the discard was skipped, leaking the consumer reference in `_subscription_consumers` forever (`cleanup_orphan_consumers` exists to sweep these but is never called anywhere). Now nested in its own `finally` so it always runs.
