...
| E11 | Handle browser back/forward (popstate) | `[]` | Mount (listener); fires on navigation | `activeRun, runLiveMessages, runSubagentCards, optimisticMessages, runs, currentRunId, runCheckpointSnapshots, threadId` + refs | No | Yes (bulk reset) |
| E12 | **Dead** (empty body) | `activeRun, apiUrl` | On change (no-op) | Nothing | No | No |
| E13 | Live → persisted handoff once the run's snapshot is loaded: releases `currentRunId` so M7/`subagentCardsForMessage` switch from the *live-run* branch to the *retained-bucket* branch (`persistedOrLive`). Does **not** clear `runLiveMessages`/`runSubagentCards` — those are per-run and outlive the handoff. | `currentRunId, currentRunSnapshotLoaded, currentRunStatus, stream.isLoading, threadId` | On run status/snapshot/loading change | `currentRunId` (+ log) | No | Yes |
| E14 | On terminal lifecycle event: mark run terminal + drop stale snapshot + refresh persisted runs (does **not** clear the run's retained `runLiveMessages`/`runSubagentCards` — kept so the run doesn't blank while its snapshot refetches) | `currentRunId, stream.debugEvents, threadId` | When debug events / current run change | `activeRun, runs, runCheckpointSnapshots` + refs | **Yes** (`refreshRuns`) | Partial |
...
| E2 | None — writes `runLiveMessages`/`optimisticMessages`, neither in deps; `sameMessageList` bails the state update when the current run's slice is unchanged. | Mild: reads latest `stream.messages`; optimistic filter only applies when `currentRunId !== null`. |
| E2b | None — `sameSubagentCards` (structural compare) bails the state update when the current run's cards are unchanged, since `liveRunSubagentCards` is a new array reference every render regardless of content. | None. |
| E3 | None | None. |
| E4 | None | None. |
...
| E12 | None | None (dead). |
| E13 | **Self-referential but convergent** — clearing `currentRunId` re-runs the effect, but the `if (!currentRunId …) return` guard stops further action. | Waits for `currentRunSnapshotLoaded` before releasing `currentRunId` for a persisted run, so `subagentCardsForMessage`/`actionsForMessage` never drop to an empty live branch before the snapshot (or the retained `runSubagentCards` fallback) is ready. It does not itself touch `runLiveMessages`/`runSubagentCards` — those are read via `buildRunMessageEntries`/`persistedOrLive`, which are indifferent to whether `currentRunId` still points at the run. |
| E14 | **Self-referential but convergent** — writes `runs`/`runCheckpointSnapshots` and reads `debugEvents`, but `handledTerminalRunIdsRef` makes terminal handling fire-once per run. It no longer clears `currentRunId` (E13 owns that) or the run's `runLiveMessages`/`runSubagentCards`, so it does not blank the transcript or the run's subagent cards. | **Yes** — races with E15/E16 discovery and with a new `submit`; mitigated by `handledTerminalRunIdsRef` (once), `joinedRunIds` cleanup, and a guarded `refreshRuns`. Marks the run terminal in `runs` so E10 re-hydrates its final snapshot; until that lands, `persistedOrLive` serves the retained live data instead of an empty result. |
